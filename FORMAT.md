...
| | |
|---|---|
| 編碼 | **UTF-8**。解不出 UTF-8 的檔案會被整個拒收。開頭的 BOM 會被自動去掉，**放了也讀得到**（但沒有理由放） |
| 行尾 | **LF（`\n`）**。純文字格式的 manifest 必須從檔案開頭起、正好是 `---` 加換行；**CRLF（`\r\n`）會讓整份 manifest 讀不到**，檔案就變成「不是一個有效的分享檔」 |
| 開頭 | `---` 之前不能有空行、空白或註解（BOM 除外） |

會讓你踩到的只有行尾。Windows 上編輯時請確認編輯器是 LF；`.gitattributes` 裡如果有 `* text=auto` 而你的工作目錄是 CRLF，推上去的內容通常仍是 LF（Git 會轉回來），但**直接用網頁介面貼上、或用會強制 CRLF 的工具產生**就會壞。不確定的話跑一次 §5 的檢查工具。
...
| 某個檔案沒出現、也沒被算進任何數字 | 副檔名不對（§2.3）——掃描器連看都沒看它 |
| 檔案被算進「需要更新 App」 | 這個數字**不只是未來型別**——凡是 App 打不開的都算進去：CRLF、少了結尾 `---`、`format` 不是 `1`、`type` 和副檔名對不上、KB 包少了 `manifest.json` 或條目不在 `entries/`。**看到這個數字先當成「我的檔案壞了」**，跑一次 `--check` |
